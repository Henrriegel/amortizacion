...
<div class="grid-container">
  ...
  <div class="lifeCycle">
    <table border="1" id="lifeCycleTable" ref="exp_table">
      <thead>
        <tr>
          <th>Periodo</th>
          <th>Saldo</th>
...
<script>
  ...
  export default {
    ...
    methods: {
      formSubmit() {
          //Si capitalizable mensualmente significa que los pagos serán mensuales
          ...
          if (isNaN(this.P) || isNaN(this.n) || isNaN(this.i)) {
              ...
              if (this.creditForm.capitalizable == "Mensualmente") {
                  if (this.creditForm.tiempoPlazo == "Años")
                      this.n *= 12;
                  if (this.creditForm.interesTiempo == "Anual")
                      this.i /= 12;
              }
              else {
                  if (this.creditForm.tiempoPlazo == "Meses")
                      this.n /= 12;
                  if (this.creditForm.interesTiempo == "Mensual")
                      this.i *= 12;
              }
      ...
      fillTable() {
        ...
        let lastRow = 0;
        this.P_anterior = this.P;
        for (let i = 1; i <= this.n; i++) {
            lastRow = table.rows.length;
            let rowInsert = table.insertRow(lastRow);
            let colPeriodo = rowInsert.insertCell(0);
            let colSaldo = rowInsert.insertCell(1);
            let colInteres = rowInsert.insertCell(2);
            let colIVA = rowInsert.insertCell(3);
            let colAbono = rowInsert.insertCell(4);
            let colPago = rowInsert.insertCell(5);
            colPeriodo.innerHTML = i;
            colInteres.innerHTML = (this.P_anterior * this.i).toFixed(2);
            colIVA.innerHTML = (this.P_anterior * this.i * 0.16).toFixed(2);
            colAbono.innerHTML = (this.a - this.P_anterior * this.i - this.P_anterior * this.i * 0.16).toFixed(2);
            colPago.innerHTML = (this.a).toFixed(2);
            this.P_anterior -= this.a - this.P_anterior * this.i - this.P_anterior * this.i * 0.16;
            colSaldo.innerHTML = (this.P_anterior).toFixed(2);
        }
